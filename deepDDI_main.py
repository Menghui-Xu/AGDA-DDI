...
import os, pandas as pd, numpy as np, torch, umap
from sklearn.decomposition import PCA
...
from deepDDI_model import Model, get_edge_index, Model_avg, Model_sum, Model_wo_feat, Model_wo_diffgraph, Model_wo_relgraph, \
...
def get_straitified_data(ratio=0.2):
    df_data = pd.read_csv('deepDDI/KnownDDI.csv')
...
feat_mat_chem, sim_mat_chem = load_feat('deepDDI/chem_Jacarrd_sim.csv')  # Load drug attribute file
...
train_feat_mat = torch.hstack([feat_mat_chem])#水平堆叠所有特征矩阵，形成一个综合的特征矩阵 train_feat_mat。
...
def generate_edges_from_similarity(train_edges, similarity_matrix,   threshold):
    ...
    # print(train_edges[0],similarity_matrix[0])
...
if args.threshold == 1:
    ...
    # print(train_edges[0], sim_mat_chem.cpu().numpy()[0])
    ...
    new_train_edges = np.vstack([train_edges, new_edges_chem])
    # new_train_edges = train_edges
...
def train_():
    ...
    for epoch in range(num_epochs):
        ...
        print(f"epoch {epoch + 1}/{num_epochs};trainging loss: {running_loss / n_iterations:.4f} ;training set acc: {running_correct / total_samples:.4f}")

        if (epoch + 1) % 50 == 0:

            print('---- 86类可视化 ----')
            save_viz_csv_86(model, test_edges, test_labels, device, epoch=epoch + 1, sample_k=10000)
...
def test_():
    event_num = 86        # 类别的数量，这里假设有65个不同的事件或类别
    ...
    with torch.no_grad():  # 确保在评估过程中不计算梯度，节省计算资源。
        ...
        assert len(total_pred) == len(total_labels), "预测和标签的样本数不一致！"
        pred_type = np.argmax(total_pred, axis=1)  # 获取预测类别 ([[0.2, 0.8], [0.6, 0.4], [0.3, 0.7]]) # 预测概率
        # print(f"total_pred shape: {total_pred.shape}, total_labels shape: {total_labels.shape}")
        ...
        #案例分析数据
        save_case_study_data(test_edges=test_edges,total_labels=total_labels,total_pred=total_pred,train_edges=train_edges)
...
def save_case_study_data(test_edges, total_labels, total_pred, train_edges,
                         topk_class=10, topk_global=20, out_dir='case_data'):
    """
    保存高置信与 zero-shot 数据
    参数均为 numpy 数组或 list，可脱离主脚本独立调用
    """
    ...
    # 3. 保存
    with open(os.path.join(out_dir, 'deepDDI_high_conf_top5_per_class.pkl'), 'wb') as f:
        pickle.dump(high_conf, f, protocol=4)

    with open(os.path.join(out_dir, 'deepDDI_high_conf_top20_global.csv'), 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Drug1', 'Drug2', 'TrueLabel', 'Prob'])
        writer.writerows(global_top20)

    with open(os.path.join(out_dir, 'deepDDI_zero_shot_data.pkl'), 'wb') as f:
        pickle.dump({'zs_pred': zs_pred, 'zs_label': zs_label}, f, protocol=4)

    print(f'[INFO] case-study data saved -> {out_dir}')
#
#
#
#
...
#
# # ==================== 86 类专用可视化 ====================
# def vis_attn_86(model, train_edges, train_labels, device, epoch,
#                 sample_k=200, out_dir='vis/deepDDI/attn'):
#     os.makedirs(out_dir, exist_ok=True)
#     model.eval()
#     with torch.no_grad():
...
#         beta_np   = beta_pair.squeeze(-1).cpu().numpy()
...
#         plt.title(f'Attention (Top-10/86)  Epoch-{epoch}')
#         plt.tight_layout()
...
#
#
# def vis_umap_86(model, train_edges, train_labels, device, epoch,
#                 sample_k=2000, out_dir='vis/deepDDI/umap'):
#     os.makedirs(out_dir, exist_ok=True)
#     model.eval()
#     with torch.no_grad():
#         perm  = torch.randperm(len(train_edges))[:sample_k]
...
#                               c=labels, s=8, cmap='tab20', vmin=0, vmax=86)
#         plt.colorbar(scatter, fraction=0.03)
#         plt.title(f'UMAP (86-class)  Epoch-{epoch}')
#         plt.tight_layout()
...
@torch.no_grad()
def save_viz_csv_86(model, edges, labels, device, epoch,
                    out_dir='vis/deepDDI', sample_k=10000):
    """
    为 DeepDDI（86 类）保存可视化基础数据 → CSV
    参数
    ----
    edges  : np.ndarray (M,2)  测试边
    labels : np.ndarray (M,)   真实 86 类标签
    sample_k: int  采样数量，None=全量；191k 建议 10k
    """
    os.makedirs(out_dir, exist_ok=True)
    model.eval()
    ...
    # 3. 直接复用模型前向 → 得到 86 维 logits
    y_pred, _ = model(edge_index, diff_edge_index, edge_type,
                      train_feat_mat,
    ...
    # 4. 提取模型内部拼接后的 512 维边向量（drug_pair）
    ae_out, _ = model.ae(train_feat_mat)
    d_g_out = model.diff_gnn(ae_out, diff_edge_index)
    o_g_out = model.orgin_gnn(ae_out, edge_index, edge_type)
    ...
    pair_emb = torch.hstack([emb[h], emb[t]]).detach().cpu().numpy()  # ← 512 维
    ...
    print(f'[VizCSV-86] 已保存 {csv_path}  ({M} 条边)')
