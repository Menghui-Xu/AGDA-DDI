...
from deepDDI_model import Model, get_edge_index
...
parser.add_argument('--rels', type=int, default=86, help='num of relations')
...
train_feat_mat = torch.hstack([feat_mat_chem])
...
for k in range(0, 5):
    ...
    with torch.no_grad():
        ...
        # class_results_df.to_csv(f'1deepDDI_class_metrics_fold_{k}.csv', index=False)
...
pd.DataFrame(results,
             columns=['fold', 'acc', 'aupr', 'auroc', 'f1', 'precision', 'recall', 'f1_micro', 'precision_micro',
                      'recall_micro']).to_csv('1deepDDI_cross_validation.csv')
